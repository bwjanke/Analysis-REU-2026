function [r, pol, res, zer, zj, fj, wj, errvec, wt, r_hist, p_hist, q_hist, pol_hist, res_hist, zer_hist, zj_hist, fj_hist, wj_hist] = aaa_history(F, varargin)
...
%   - 't0', T0: force the first AAA support point to be the sample
%       point nearest T0.
%
...
[F, Z, M, dom, tol, mmax, cleanup_flag, cleanup_tol, needZ, mmax_flag, ...
    nlawson, dampratio, degree_flag, degree, sign_flag, deriv_deg, t0] ...
...
pol_hist = cell(mmax,1);
res_hist = cell(mmax,1);
zer_hist = cell(mmax,1);
...
for m = 1:mmax
    ...
    if (m == 1) && ~isempty(t0)
        ...
        % choose sample point nearest t0
        [~, jj] = min(abs(Z(J) - t0));

    else

        % standard AAA greedy selection
        [~, jj] = max(abs(F(J) - R(J)));
    ...
    % Rational approximant at this step
    r_hist{m} = @(zz) reval(zz, zj_m, fj_m, wj_m);
    
    % Numerator and denominator handles
    p_hist{m} = @(zz) bary_num(zz, zj_m, fj_m, wj_m);
    q_hist{m} = @(zz) bary_den(zz, zj_m, wj_m);
    
    % Save barycentric data
    zj_hist{m} = zj_m;
    fj_hist{m} = fj_m;
    wj_hist{m} = wj_m;
    
    % Save poles/residues/zeros
    [pol_m, res_m, zer_m] = prz(zj_m, fj_m, wj_m);
...
% trim to how many iterations actually went through
r_hist   = r_hist(1:m);

p_hist   = p_hist(1:m);
q_hist   = q_hist(1:m);

pol_hist = pol_hist(1:m);
res_hist = res_hist(1:m);
zer_hist = zer_hist(1:m);
...
function [F, Z, M, dom, tol, mmax, cleanup_flag, cleanup_tol, ...
    ...
    deriv_deg, t0] = parseInputs(F, varargin)
...
while ( ~isempty(varargin) )   % Check if parameters have been provided
    ...
    elseif ( strncmpi(varargin{1}, 't0', 2) )
        if ( isnumeric(varargin{2}) && isscalar(varargin{2}) )
            t0 = varargin{2};
        else
            error('AAA:t0', 't0 must be a scalar.');
        end
        varargin([1,2]) = [];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   AAA_AUTOZ   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [r, pol, res, zer, zj, fj, wj, errvec, wt, ...
          r_hist, p_hist, q_hist, ...
          pol_hist, res_hist, zer_hist, ...
          zj_hist, fj_hist, wj_hist] = ...
    aaa_autoZ(F, dom, tol, mmax, cleanup_flag, cleanup_tol, mmax_flag, ...
               nlawson, dampratio, degree_flag, degree, sign_flag)
...
for n = 5:14
    ...
    if degree_flag
       [r, pol, res, zer, zj, fj, wj, errvec, ...
         wt, r_hist, p_hist, q_hist, ...
         pol_hist, res_hist, zer_hist, ...
         zj_hist, fj_hist, wj_hist] = aaa_history(F, Z, 'tol', tol, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   BARY_NUM   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function p = bary_num(zz, zj, fj, wj)
% Evaluate numerator of barycentric rational form:
%
%   p(z) = sum_j w_j f_j / (z - z_j)

zv = zz(:);
CC = 1./(zv - zj.');
p = CC * (wj .* fj);

% Handle z = support point exactly:
for k = 1:length(zj)
    idx = (zv == zj(k));
    if any(idx)
        p(idx) = Inf;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   BARY_DEN   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function q = bary_den(zz, zj, wj)
% Evaluate denominator of barycentric rational form:
%
%   q(z) = sum_j w_j / (z - z_j)
